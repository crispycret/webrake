...
from . import LazyUrl, is_type_of
from . import make_get_has_header_methods_for_obj_with_response
...
class HistoryEntry(object):
	...
	def __init__(self, url, response, timestamp=None):
		...
		if type(url) is not LazyUrl:
			url = LazyUrl(url)
	
		is_type_of(requests.Response, response)

		self.url = url
		self.response = response
		self.timestamp = timestamp

		make_get_has_header_methods_for_obj_with_response(HistoryEntry)
